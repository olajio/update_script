...
def retry_fetch_thread(initial_host_list, systemtype, domain):
    ...
    # Set up auth for the retry loop — use PAT from environment
...
def main(systemtype, domain):
    ...
    if not github_pat:
        print(f"{datetime.datetime.utcnow().isoformat()} Error: GITHUB_PAT environment variable not set. Exiting.")
        logger.error("GITHUB_PAT environment variable not set. Exiting.")
